...
public class Server{
	...
	private LinkedBlockingQueue<MsgStore> messages;//Msg stored with index of clientList attached
	...
	public void ReadUsers(){
		try{
			...
			//				System.out.println("Print: "+Users.get(i).name+" "+Users.get(i).password);
		}catch(Exception e){
			e.printStackTrace();
		}	
	}
	...
	/**
	 * The Server class is here
	 * It separates the work into four purposes
	...
	/**
	 * the below function deals with login/blocking/
	 * whoelse/wholasthr command and return a string to send and send it
	 * The cool thing is that it also spawns reading and writing thread
	 * */
	public class ConnectionToClient {
		 ...
		 * */
		ConnectionToClient(final Socket socket,final int index) throws IOException {
			this.socket = socket;
			this.index=index;
			if (!isBlocked(socket.getInetAddress().getHostAddress())){
				...
				try{
					while(countInput++<3&&!(login=isUser(promptName,promptKey))){
				...
				if (!lockthisip){
					TimerReset(timer, oneHour);// start the clock for one hour count
					...
					Thread read = new Thread(){
						public void run(){
							try{
								out.writeObject("Welcome, "+username+"! Please start typing!");
							}catch(Exception e){
								System.err.println("Welcome messgae not sent successfully!");
							}
							while(true){
								try{
									if (!socket.isClosed()){// check socket closed
										if (socket.isConnected()){//check connected
											Object obj;	
											try{
												obj= in.readObject();
											}catch (Exception e){
												System.err.println("Can't readObject! Pipe broken.");
												connAlive=false;
												break;
											}
											if (obj==null){
											...
											messages.add(new MsgStore(obj,index));
											//System.err.println("Hi, added to messages: "+obj.toString());
											//sendToAll(obj);
										}else if (!socket.getInetAddress().isReachable(1000)){// check reachability
											System.out.println("Can't wait longer to get it back!");
											connAlive=false;
											break;

										}else{
											System.out.println("the input from client is out!");
											socket.close();
											connAlive=false;
											break;
					...
					synchronized (this) {// entity operation
						...
						int rightIndex=-1;// consider size()==0 and size()==1 the index is different
						if (thisIndex!=0)
							rightIndex=thisIndex-1;
						else rightIndex=thisIndex;
						BlockedIP.add(socket.getInetAddress().getHostAddress());
						System.err.println("The ip added is :"+BlockedIP.get(rightIndex));
						for (int j=0;j<BlockedIP.size();j++)
							System.err.println("The all Blocked IP's: "+BlockedIP.get(j));
						connAlive=false;
						try{
							/** this part is to eliminate the blocking 60 seconds latter**/
							...
							thisTimer.schedule(new TimerTask(){
								...
								public void run() {
									...
									BlockedIP.remove(socket.getInetAddress().getHostAddress());
								}

							}, 60*1000);// remove from block list after 60 seconds
						}catch(Exception e){System.err.println("Can't remove the ip from block list after 60s!");}
		...
		public void write(Object obj) {
			...
			catch(IOException e){ System.err.println("Write received an null pointer!"); }
		}
		/**
		...
		/**
		 * the below function deals with wholasthr command and return a string to send
		 * */
		...
		public String echo(MsgStore message){
			...
			}else if (tempMess.length()!=4&&!tempMess.substring(4, 5).equals(" ")){
		...
		public String broadcast(MsgStore message){
			...
			}else if (tempMess.length()!=9&&!tempMess.substring(9, 10).equals(" ")){
				return "Sorry, I don't recognized your input!"+'\n';
			}
			else{
			String temp="";
			try{
				//temp="Echo: "+tempMess.substring(4);
				//sendToOne(message.index,temp.trim());
				temp+="Message Broadcasted: " + tempMess.substring(9).trim();
			}catch(Exception e){
	...
	/**
	 * Send the object to the client handler indicated by the index in the clientList 
	 * **/
	public void sendToOne(int index, Object message)throws IndexOutOfBoundsException {
		//clientList.get(index).client.write(message);
		int i=index;

		System.out.println("This is send to One received: "+message.toString());
		if (message!=null)	{	

			ConnectionToClient client=clientList.get(i).client;
			if (!client.socket.isClosed()&&client.socket.isConnected()&&client.connAlive){
				System.err.println("Write to "+i+"th node"+" total: "+clientList.size());
				client.write(message);
			}else if (client.myage >0&&client.connAlive){
				// This handles the case the connection is older than 1 hour
				// They are removed
				System.err.println("Connection to client "+i+" is offline and older than 1 hour!");
				clientList.remove(i);
				i--;
			}
	...
	public void sendToAll(Object message){
		int i=0;
		System.out.println("This is send to ALL received: "+message.toString());
		if (message!=null)	{	
			for(i=0 ;i<clientList.size();i++){
				...
				if (!client.socket.isClosed()&&client.socket.isConnected()&&client.connAlive){
					...
					// This handles the case the connection is older than 1 hour
					// They are removed
					System.err.println("Connection to client "+i+"is older than 1 hour!");
					clientList.remove(i);
					i--;
				}
			} 
